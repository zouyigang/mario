...
DEATH_REWARD_THRESHOLD = -15      # 原始 reward <= 此值视为死亡步
DEATH_PENALTY_SEEN = 15           # 死亡步惩罚
# 接着训：checkpoint 路径（可改为 checkpoints/mario_XXX_steps.zip 指定某一轮）
LOAD_CHECKPOINT = os.path.join("sb3_mario_logs", "best", "best_model.zip")
# 本轮再训练的环境总步数
ADDITIONAL_TIMESTEPS = 4_000_000   # 接着训总步数；可与 train_sb3.py 的 TOTAL_TIMESTEPS 量级对照调整
# 加载后覆盖到模型上的熵系数与学习率（须与 checkpoint 算法一致）
ENT_COEF_CONTINUE = 0.02            # 熵系数
LR_CONTINUE = 3e-5                # 学习率（线性衰减时的起始值）
LR_CONTINUE_END = 1e-5              # 训练末期学习率；线性衰减让后期更新变小，利于收敛、减少震荡
USE_LR_DECAY_CONTINUE = True        # True=学习率从 LR_CONTINUE 线性降到 LR_CONTINUE_END；False=恒定 LR
...
PPO_N_EPOCHS = 3                    # 每批 rollout 重复训练轮数
PPO_CLIP_RANGE = 0.18               # 策略更新裁剪
# 早停：当 rollout 平均奖励相对「历史最高」明显下降时提前结束，保留峰值附近的策略
EARLY_STOP_ENABLED = False          # 是否启用早停
EARLY_STOP_RATIO = 0.90             # 当前奖励 < 历史最高 * 此比例 时计一次「下降」
EARLY_STOP_PATIENCE = 4             # 连续多少次「下降」后停止
...
EVAL_FREQ = 20_000                  # 评估间隔（步）
CHECKPOINT_FREQ = 50_000            # 按「环境总步数」计：每隔多少步保存一次 checkpoint（多并行 env 时下面会除以 NUM_ENVS 换算成 callback 频率）
