...
FLAG_GET_BONUS = 50       # 通关仍应明显优于任意死局（本配置下 50+32）
# 迷宫探路时横向位移常小：过重会逼智能体送死重开；宜轻罚或仅作防刷分信号
NO_PROGRESS_PENALTY_AFTER = 48   # 窗口拉长，减少「试管道」被判慢速
NO_PROGRESS_MIN_DX_IN_WINDOW = 24 # 略放宽；过小仍易在直线关尾误判，可按关卡再调
NO_PROGRESS_PENALTY_SEEN = 0.2   # 每步轻扣；原 1.5 在迷宫里数百步会碾压死亡惩罚
# 步数惩罚过重时，长局探索总回报会低于短局送死
STEP_PENALTY_SEEN = 0.02         # 轻量；需强催速可提到 0.05～0.1
...
BACKTRACK_GRACE_STEPS = 2
BACKTRACK_SINGLE_STEP_MAX = 200
