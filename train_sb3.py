...
DEATH_PENALTY_SEEN = 32           # 死亡步惩罚；与正常步 ±1 不要差距太大，否则方差太大 value function 难学
# 总训练步数
...
DYN_ENT_NO_PROGRESS_STEPS = 300_000  # frame_skip 2 下等效物理时间与原 150k@skip4 一致
DYN_ENT_BOOST_STEP = 0.01      # 每次提升量
...
DEAD_LOOP_PENALTY_SEEN = 20   # 迷宫死循环截断终局惩罚；与 train_sb3_continue 保持一致，略小于死亡惩罚
...
MAX_CLAWBACK = 60.0                 # 从 30 → 60；最大惩罚=40+60=100，远超错路探索总收益(~30-60)
...
FRONTIER_BONUS = 0.3         # 邻格有未访问格时小奖励，吸引靠近边界
# ---- 纵向探索奖励（鼓励往下跳到新层，4-4 迷宫正确路径关键）----
Y_LAYER_BONUS = 3.0          # 首次到达新 Y 层（每 32 像素为一层）时额外奖励，引导向下探索
Y_LAYER_SIZE = 32            # Y 层划分粒度（像素）；NES 马里奥纵向跨层约 32px
...
def main():
    ...
    if DYN_ENT_ENABLED and ALGORITHM.upper() == "PPO":
        ...
        print("动态熵系数已启用：base={} min={} max={} 无进展{}步触发".format(
            ENT_COEF, DYN_ENT_MIN, DYN_ENT_MAX, DYN_ENT_NO_PROGRESS_STEPS))
