...
TELEPORT_ESCALATION = 1.25
WRONG_BRANCH_STEP_CLAWBACK = 0.5
MAX_TELEPORT_MULTIPLIER = 4.0
MAX_CLAWBACK = 25.0
...
class ClipRewardExceptDeathWrapper(Wrapper):
    ...
    2. 回传事件（teleport_branch）→ 独立惩罚，递增有上限
    3. 死循环超时（dead_loop）→ 固定惩罚
    4. 死亡（reward <= threshold 或 terminated 非过关）→ 固定惩罚
    5. 正常步 → sign 裁剪 ±1，叠加 step_penalty 和 no_progress_penalty

    优化点：
    - 递增倍率封顶至 max_teleport_multiplier（默认 4×），避免后期方差爆炸
    - 错误路段 clawback 封顶至 max_clawback（默认 25 分）
    - 走对路回绕给予 correct_wrap_bonus 正奖励，提供更强的路径探索信号
    """
    ...
    def __init__(self, env,
        ...
        self._escalation = float(teleport_escalation)
        self._max_multiplier = float(max_teleport_multiplier)
    ...
    def step(self, action):
        ...
        if is_correct_wrap:
            ...
            # 分支回传：递增惩罚（倍率封顶）+ 路段正奖励回吐（clawback 封顶）
            multiplier = min(
                self._escalation ** max(0, teleport_count - 1),
                self._max_multiplier
            )
            base_penalty = self._teleport_branch_base * multiplier
            wrong_steps = info.get("wrong_branch_steps", 0)
            clawback = min(wrong_steps * self._clawback_per_step, self._max_clawback)
            reward = -(base_penalty + clawback)
    ...
    if CLIP_REWARD:
        if CLIP_REWARD_EXCEPT_DEATH:
            env = ClipRewardExceptDeathWrapper(
				...
				max_teleport_multiplier=MAX_TELEPORT_MULTIPLIER,
