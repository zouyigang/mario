...
# 从头训 PPO 的熵系数与学习率
ENT_COEF = 0.05            # 迷宫：略提高熵以维持探索；PPO 默认 0.01
...
TELEPORT_BRANCH_BASE_PENALTY = 5   # 分支回传基础惩罚（固定，不递增）；略降以减少「不敢靠近路口」
...
MAZE_STALL_PENALTY = 0.2     # 连续停在同一格的惩罚；与 continue 脚本一致
MAZE_STALL_ESCALATE_PER_STEP = 0.015  # 同格停留越久惩罚线性升高（上限见下）
...
MAZE_DEAD_LOOP_STEPS = 150   # 缩短使终局惩罚在 γ 贴现下仍「可感知」
