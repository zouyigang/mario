...
# 超参数
# ======================
MARIO_ENV_ID = "SuperMarioBros-2-1-v1"   # 训练 1-2 关；可改为 1-1, 2-1 等
...
DEATH_PENALTY_SEEN = 40           # 死亡步智能体看到的惩罚；加大可让智能体更忌惮掉坑/撞怪，通用关卡更稳（正常步 ±1，过关约 +16）
# 总训练步数
TOTAL_TIMESTEPS = 100_000_000   # 激进版：可改为 5_000_000 等更长训练
# 从头训 PPO 的熵系数与学习率
ENT_COEF = 0.8             # 熵系数，越大探索越强
LR = 3e-4                   # 学习率
...
DEAD_LOOP_PENALTY_SEEN = 10   # 应比 DEATH_PENALTY_SEEN 小，让"卡住"不如"死亡"严重，但比普通 -1 重
...
FLAG_GET_BONUS = 50       # 与 DEATH_PENALTY_SEEN 同量级，保证过关动机 ≈ 避免死亡动机
