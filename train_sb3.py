...
NO_PROGRESS_MIN_DX_IN_WINDOW = 50  # 窗口内至少前进多少像素才不算无进展（约 0.47 px/步）。小跳蹭步在关尾会低于此值，通用各关卡
NO_PROGRESS_PENALTY_SEEN = 0.8   # 每步扣除的奖励（小负值，避免与死亡惩罚混淆；0.5 让前进+1 变 0.5，原地 0 变 -0.5）
# 每步时间/步数惩罚：每步额外扣除此值，步数越多总奖励越低，促使尽快过关（0=关闭）
STEP_PENALTY_SEEN = 0.05          # 如 0.1：500 步多扣 50 分，600 步多扣 60 分；步数多总奖励明显降低
...
class ClipRewardExceptDeathWrapper(Wrapper):
    ...
    优先级（从高到低）：
    1. 回传事件（teleport_immediate / teleport_branch）→ 独立惩罚，递增机制
    2. 死循环超时（dead_loop）→ 固定惩罚
    3. 死亡（reward <= threshold 或 terminated 且非过关）→ 固定惩罚
    4. 正常步 → sign 裁剪 ±1，叠加 step_penalty 和 no_progress_penalty

    回传惩罚优先于死亡判定，避免 TeleportBackDetector 修改的 raw reward 误触死亡阈值。
    分支回传额外扣除「错误路段正奖励」：走错路拿到的 +1/步全部回吐。
    """
    ...
    def __init__(self, env, death_threshold=-15, death_penalty_seen=15,
                 ...
                 teleport_immediate_penalty=20, teleport_branch_base_penalty=8,
                 teleport_escalation=1.5, wrong_branch_step_clawback=0.8):
        super().__init__(env)
        ...
        self._teleport_immediate_penalty = float(teleport_immediate_penalty)
        self._teleport_branch_base_penalty = float(teleport_branch_base_penalty)
        self._teleport_escalation = float(teleport_escalation)
        self._wrong_branch_step_clawback = float(wrong_branch_step_clawback)

    def step(self, action):
        obs, reward, terminated, truncated, info = self.env.step(action)

        is_teleport_immediate = info.get("teleport_immediate", False)
        is_teleport_branch = info.get("teleport_branch", False)
...
def make_env(env_id=None):
    ...
    if ENABLE_TELEPORT_DETECTION:
        env = TeleportBackDetector(
            ...
            immediate_teleport_dx=TELEPORT_IMMEDIATE_DX,
    ...
    if CLIP_REWARD:
        if CLIP_REWARD_EXCEPT_DEATH:
            env = ClipRewardExceptDeathWrapper(
                ...
                teleport_immediate_penalty=TELEPORT_IMMEDIATE_PENALTY,
                teleport_branch_base_penalty=TELEPORT_BRANCH_BASE_PENALTY,
                teleport_escalation=TELEPORT_ESCALATION,
