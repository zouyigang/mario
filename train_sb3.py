...
DYN_ENT_ADJUST_SPEED = 0.10    # v8.9：0.15→0.10，EMA 更平滑，每次窗口变化幅度更小
...
# 阶梯里程碑（depth_pixels, bonus）：让"尝试"中途死也能吃到一部分
...
class PostLayerLeftBonusWrapper(Wrapper):
    ...
    触发：上游 CellExplorationWrapper 写出 info["new_y_layer"]=True
    窗口期内（默认 60 步）：
      - _min_x 跟踪本窗口最深左位
      - 每步若 current_x < _min_x → 累加 per_pixel × delta + 跨档里程碑 bonus
      - current_x > entry_x → 已重回入口右侧，关闭窗口
      - 步数耗尽 → 关闭
    活动窗口内若 new_y_layer 再触发：
      - current_x < entry_x（向左下到了新层）→ 给 new_layer_left_bonus，并以新位置重开窗口
      - current_x >= entry_x（同位置/右侧的小跳）→ 不重开（防薅）
    bonus 通过 info 传递，由 ClipRewardExceptDeathWrapper 在最后无条件加上。
    """
        ...
        # 优先级 6：正常步
            ...
            if info.get("new_cell", False):
                ...
                if reward >= 0:
                    ...
                    # 负向 env reward + new_cell + 处于后退中 → 给个小正floor，避免被环境拖到负
