...
TELEPORT_PENALTY_IMMEDIATE = 20   # 立即回传惩罚（如下管后立即被传回，比死亡更严重）
TELEPORT_PENALTY_BRANCH = 10      # 分支回传惩罚（走错分支后走了一段路被传回）
...
def make_env(env_id=None):
    ...
    # 回传检测：识别并惩罚两种回传状态（立即回传和分支回传）
    if ENABLE_TELEPORT_DETECTION:
        env = TeleportBackDetector(
            env,
            teleport_penalty_immediate=TELEPORT_PENALTY_IMMEDIATE,
            teleport_penalty_branch=TELEPORT_PENALTY_BRANCH,
